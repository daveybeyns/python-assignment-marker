    ...
    "rubric": [
        {
            ...
            "levels": {
                "0": "No working program submitted.",
                "1": "A limited attempt runs, but the main program does not work correctly.",
                "2": "The program runs partly but contains errors affecting several results or repetitions.",
                "3": "The program mostly works, with only a minor error or omission.",
                "4": "The complete program runs correctly for suitable test values, repeats when requested and closes correctly."
            }
        },
        {
            "id": "inputCasting",
            ...
            "levels": {
                "0": "No meaningful user input.",
                "1": "Limited use of input() with major errors.",
                "2": "Inputs are present but one required input or conversion is missing or unsuitable.",
                "3": "All required inputs are present, with a minor prompt or data-type issue.",
                "4": "Weekly saving, number of weeks and repeat response are requested clearly; numeric inputs use suitable casting."
            }
        },
        {
            "id": "forRange",
            ...
            "levels": {
                "0": "No meaningful for loop.",
                "1": "A limited or incomplete attempt at a for loop.",
                "2": "A for loop is present but repetitions are not reliably correct.",
                "3": "A suitable for loop is used with a minor range or week-number issue.",
                "4": "A correct for loop and range() repeat exactly once for each week and produce correct week numbers."
            }
        },
        {
            "id": "whileRepeat",
            ...
            "levels": {
                "0": "No meaningful while loop.",
                "1": "A limited or incomplete attempt at a while loop.",
                "2": "A while loop is present but does not reliably repeat or stop correctly.",
                "3": "The while loop mostly works, with a minor response-handling or placement issue.",
                "4": "A correct while loop repeats the whole savings tracker when the user enters yes and stops for no."
            }
        },
        {
            "id": "runningTotal",
            ...
            "levels": {
                "0": "No meaningful running total.",
                "1": "Limited attempt to calculate a running total.",
                "2": "A total variable is used but some weekly or final results are incorrect.",
                "3": "The running total is mostly correct, with one minor calculation or reset issue.",
                "4": "The total resets for each plan, updates accurately each week and produces correct final results for all tests."
            }
        },
        {
            "id": "output",
            ...
            "levels": {
                "0": "No meaningful output or readable code.",
                "1": "Output is difficult to understand or code is poorly organised.",
                "2": "Output is understandable but inconsistent; code quality or indentation needs improvement.",
                "3": "Output and code are clear, with only minor formatting or organisation issues.",
                "4": "Output is clear and well formatted; indentation, variable names and code organisation are consistently appropriate."
            }
        }
    ],
    "testCases": [
...
function assess(raw, helpers, source, assignment) {
    ...
    function orderedWeekProgress(lines, plans) {
        let position = 0;
        for (const plan of plans) {
            for (let week = 1; week <= Number(plan.weeks); week += 1) {
                const expectedTotal = Number(plan.weekly) * week;
                let found = false;
                while (position < lines.length) {
                    const line = lines[position++];
                    if (
                        line.toLowerCase().includes("week")
                        && helpers.hasNumber(line, week)
                        && helpers.hasNumber(line, expectedTotal)
                    ) {
                        found = true;
                        break;
                    }
    ...
    function orderedFinalTotals(lines, plans) {
        let position = 0;
        for (const plan of plans) {
            const expectedTotal = Number(plan.weekly) * Number(plan.weeks);
            let found = false;
            while (position < lines.length) {
                const line = lines[position++];
                ...
                if (
                    !lower.includes("week")
                    && ["final", "total", "saved", "saving"].some((word) => lower.includes(word))
                    && helpers.hasNumber(line, expectedTotal)
                ) {
                    found = true;
                    break;
    ...
    raw.tests = (raw.tests || []).map((test, index) => {
        ...
        const plans = testCase.expected?.plans || [];
        const lines = outputLines(test.output);
        const outputLower = String(test.output || "").toLowerCase();
        const allInputsUsed = (test.inputsUsed?.length || 0) === (testCase.input?.values?.length || 0);
        const weeklyProgress = orderedWeekProgress(lines, plans);
        const finalTotals = orderedFinalTotals(lines, plans);
        const closingWords = ["closed", "goodbye", "thank", "finished", "complete", "bye"];
        const closing = Boolean(lines.length)
            && closingWords.some((word) => lines[lines.length - 1].toLowerCase().includes(word));
        const multiplePlans = plans.length > 1;
        const repeatWorked = !multiplePlans || (allInputsUsed && finalTotals && weeklyProgress);
        let resetWorked = true;
        if (multiplePlans) {
            const combinedTotal = plans.reduce(
                (sum, plan) => sum + Number(plan.weekly) * Number(plan.weeks), 0
            );
            const secondTotal = Number(plans[plans.length - 1].weekly)
                * Number(plans[plans.length - 1].weeks);
            const finalLines = lines.filter((line) => {
                const lower = line.toLowerCase();
                return !lower.includes("week")
                    && ["final", "total", "saved", "saving"].some((word) => lower.includes(word));
            });
            resetWorked = finalTotals && (
                Math.abs(combinedTotal - secondTotal) < 0.005
                || !finalLines.some((line) => helpers.hasNumber(line, combinedTotal))
            );
        }
        return {
            ...test,
            ...
            labels: {
                week: outputLower.includes("week"),
                final: ["final", "total", "saved"].some((word) => outputLower.includes(word)),
                closing
            }
        };
    });
